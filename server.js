...
// ---------------------------------------------------------------
// TEMPORARY full data audit \u2014 runs once at boot, reports to logs, then
// gets removed. Checks: duplicate transactions/products, stock-replay
// consistency (does chronological IN/OUT/DISCARD replay ever imply negative
// stock, even if the final total balances), retail/market price alignment,
// and date sanity.
// ---------------------------------------------------------------
try {
  console.log("\n========== FULL DATA AUDIT ==========");
  ...
  const txns = db.prepare("SELECT * FROM transactions ORDER BY product_id, timestamp").all();
  const supplierIds = new Set(suppliers.map((s) => s.id));
  const productIds = new Set(products.map((p) => p.id));

  console.log(`Scope: ${products.length} products, ${suppliers.length} suppliers, ${txns.length} transactions`);

  // 1. Duplicate transactions (same product/type/qty/price/staff within 5s)
  let dupTxnCount = 0;
  const outInMarketOrRetail = txns.filter((t) => ["OUT", "IN", "DISCARD"].includes(t.type));
  for (let i = 1; i < outInMarketOrRetail.length; i++) {
    const a = outInMarketOrRetail[i - 1], b = outInMarketOrRetail[i];
    if (a.product_id === b.product_id && a.type === b.type && a.qty === b.qty && a.price === b.price && a.staff === b.staff &&
        Math.abs(new Date(b.timestamp) - new Date(a.timestamp)) < 5000) {
      dupTxnCount++;
      console.log(`[DUPLICATE?] product=${a.product_id} type=${a.type} qty=${a.qty} price=${a.price} staff=${a.staff} | ${a.id}@${a.timestamp} vs ${b.id}@${b.timestamp}`);
    }
  }
  console.log(`1. Duplicate transactions: ${dupTxnCount} found`);

  // 2. Duplicate products (same name+category)
  const byNameCat = {};
  for (const p of products) {
    const key = `${p.name.trim().toLowerCase()}|${p.category.trim().toLowerCase()}`;
    (byNameCat[key] = byNameCat[key] || []).push(p);
  }
  const dupProducts = Object.entries(byNameCat).filter(([, l]) => l.length > 1);
  console.log(`2. Duplicate products (same name+category): ${dupProducts.length} found`);
  for (const [key, list] of dupProducts) console.log(`   "${key}": ${list.map((p) => p.id).join(", ")}`);

  // 3. Orphaned references
  let orphanTxns = 0, orphanSuppliers = 0;
  for (const t of txns) {
    if (!productIds.has(t.product_id)) orphanTxns++;
    if (t.supplier_id && !supplierIds.has(t.supplier_id)) orphanSuppliers++;
  }
  console.log(`3. Orphaned references: ${orphanTxns} transactions pointing to a deleted product, ${orphanSuppliers} pointing to a deleted supplier (expected/harmless, shown as "\u2014" in UI)`);

  // 4. Stock replay consistency \u2014 does chronological replay ever go
  // negative, even though the final total balances by construction?
  let negativeReplayCount = 0;
  const byProduct = {};
  for (const t of txns) (byProduct[t.product_id] = byProduct[t.product_id] || []).push(t);
  for (const p of products) {
    const list = (byProduct[p.id] || []).slice().sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
    const netChange = list.reduce((s, t) => s + (t.type === "IN" ? t.qty : -t.qty), 0);
    let running = p.stock - netChange; // inferred starting stock before any logged transaction
    let wentNegative = false;
    for (const t of list) {
      running += t.type === "IN" ? t.qty : -t.qty;
      if (running < -0.001) wentNegative = true;
    }
    if (wentNegative) {
      negativeReplayCount++;
      console.log(`[STOCK HISTORY ISSUE] "${p.name}" (${p.id}): chronological replay of its transactions implies negative stock at some point (likely a backdated entry logged out of order).`);
    }
  }
  console.log(`4. Products whose transaction history implies negative stock at some point: ${negativeReplayCount} found`);

  // 5. Price/price_type alignment for OUT sales
  let marketMismatch = 0, missingPriceType = 0;
  for (const t of txns) {
    if (t.type !== "OUT") continue;
    if (!t.price_type) { missingPriceType++; continue; }
    if (t.price_type === "market" && t.market_price != null && Math.abs(t.price - t.market_price) > 0.01) {
      marketMismatch++;
      console.log(`[PRICE MISMATCH] txn ${t.id}: tagged market but price=${t.price} != market_price=${t.market_price}`);
    }
  }
  console.log(`5. OUT sales tagged "market" whose charged price doesn't match the recorded market price: ${marketMismatch} found`);
  console.log(`   OUT sales missing a price_type entirely (shouldn't happen post-migration): ${missingPriceType} found`);

  // 6. Date sanity
  const now = new Date();
  let futureDates = 0, invalidDates = 0;
  for (const t of txns) {
    const d = new Date(t.timestamp);
    if (isNaN(d.getTime())) invalidDates++;
    else if (d.getTime() > now.getTime() + 60000) futureDates++;
  }
  console.log(`6. Transactions with an invalid timestamp: ${invalidDates} found`);
  console.log(`   Transactions dated in the future: ${futureDates} found`);

  // 7. Pricing sanity \u2014 retail below purchase price (selling at a loss,
  // possibly a typo) and market price lower than retail (unusual but not
  // necessarily wrong)
  let belowCost = 0, marketBelowRetail = 0;
  for (const p of products) {
    if (p.retail_price < p.purchase_price) {
      belowCost++;
      console.log(`[PRICING] "${p.name}" (${p.id}): retail price P${p.retail_price} is below purchase cost P${p.purchase_price}`);
    }
    if (p.market_price != null && p.market_price < p.retail_price) {
      marketBelowRetail++;
    }
  }
  console.log(`7. Products priced below their own purchase cost: ${belowCost} found`);
  console.log(`   Products where market price is set lower than retail price: ${marketBelowRetail} found (not necessarily wrong, just noted)`);
